...
fn create_equiv_check_file(
    ...
    // Create the file
    // TODO Need to base the imports off of what is imported by the original and
    // TODO refactored COQ files.
    info!("Directory: {:?}", dir);

    let mut equiv_check = File::create(dir.join("EquivCheck.v"))?;
    ...
    writeln!(equiv_check, "Require Import {}.", original_coq.file_stem().unwrap().to_str().unwrap())?;
    writeln!(equiv_check, "Require Import {}.", refactored_coq.file_stem().unwrap().to_str().unwrap())?;
    writeln!(equiv_check, "Require Import Coq.ZArith.ZArith.")?;
    writeln!(equiv_check, "Require Import List.")?;
    writeln!(equiv_check, "Import ListNotations.")?;
    ...
    let proof = generate_equiv_check_proof(&top_level_function, &original_coq, &refactored_coq)?;
